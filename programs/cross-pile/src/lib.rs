...
pub mod cross_pile {
    ...
    pub fn accept_challenge(
        ...
        //challengee_pub_key: Pubkey,
    ) -> ProgramResult {
        let challenge = &mut ctx.accounts.challenge;

        // should make sure no one has already accepted the challenge
        //challenge.challengee = challengee_pub_key;
        challenge.challengee = *ctx.accounts.challengee.to_account_info().key;
        Ok(())
    }
}
...
pub struct NewChallenge<'info> {
    ...
    #[account(mut)]
    pub challenge_initiator: Signer<'info>,
